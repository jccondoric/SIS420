...
def train(episodes):
    ...
    #q_table = np.full((env.observation_space.n, env.action_space.n), np.random.uniform(-1, 1))
    ...
    for i in range(episodes):
        ...
        # Reinicia el entorno en cada 1000 episodios, alternando modos con y sin 
        if (i + 1) % 500 == 0:
            env.close()
            env = gym.make('FrozenLake-v1', is_slippery=False, render_mode='human')
            # print(f'Episodio {i + 1} recompensa: {rewards_per_episode[i]}')
            # print(f'Tabla Q actualizada:{ q_table}')
        else:
            env.close()
            env = gym.make('FrozenLake-v1', is_slippery=False)
        ...
        while (not terminated and not truncated):
            ...
            # print(f'Estado: {state}, Accion: {action}, Nuevo estado: {new_state}, Recompensa: {reward}')
            # Actualiza la tabla Q con la nueva informacion obtenida
            q_table[state, action] = q_table[state, action] + learning_rate * (reward + discount_factor * np.max(q_table[new_state, :]) - q_table[state, action])
            # q_table[state, action] = q_table[state, action] + learning_rate * (reward + np.max(q_table[new_state, :]) - q_table[state, action])
            # print(f'Tabla Q actualizada: {q_table[state, action]}')
